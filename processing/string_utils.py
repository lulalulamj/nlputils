...
# num_base_pat = '((\+|-)?[0-9]+((\.?[0-9]+)|(\,[0-9]{3,3})+)?)'
# number_pat = re.compile(num_base_pat + "( ?[xX] ?{}{})?".format(num_base_pat, "(\^{})?".format(num_base_pat)))
...
def num_normalization1(string, replace="X"):
    """
...
# replace all the numbers with single pattern
# replace all the number normalize length
def num_normalization2(string, replace="X", skip_alnum=False):
    """
...
print num_normalization2("Model3 has sold 2.5 x 10^8 devices with unit price of $699.0")
print num_normalization2("Model3 has sold 2.5 x 10^8 devices with unit 3Mn4SO6 of $699.0", skip_alnum=True)
print num_normalization2("22.987654^-0.9")
print num_normalization2("Iphone3")
